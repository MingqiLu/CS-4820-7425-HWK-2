...
(defmacro or-err (saexpr body)
  (let* ((saexpr-prime
          (intern-in-package-of-symbol
       (symbol-name saexpr)
           saexpr)))
    `(let ((saval (saeval ,saexpr a)))
       (match saval
         (:rational (let ((,saexpr-prime saval))
              ,body))
         (& saval)))))
...
(definec saeval (e :saexpr a :assignment) :rat-err
  (match e
    ...
    (:usaexpr
     (('- x) (or-err x (- 0 x)))
     (('/ x) (or-err x (if (= x 0) *er* (/ x)))))
    (:bsaexpr
     ((x '- y) (or-err x (or-err y (- x y))))
     ((x '+ y) (or-err x (or-err y (+ x y))))
     ((x '* y) (or-err x (or-err y (* x y))))
     ((x '/ y) (or-err x (or-err y (if (= y 0) *er* (/ x y)))))
     ((x '^ y) (or-err x (or-err y (if (or (not (intp y)) (and (< y 0) (= x 0)))
                     *er*
                     (expt x y))))))))
...
;; helper needed
(property eval-non-rational-is-er (x :saexpr a :assignment)
  :h (not (rationalp (saeval x a)))
  :b (== (saeval x a) *er*))

;; 4. (1 / (x / y)) = (y / x), for saexpr's x, y,
(property (x y :saexpr a :assignment)
  :h (not (xor (== (saeval y a) 0) (== (saeval x a) 0)))
  :b (== (saeval `(1 / (,x / ,y)) a)
         (saeval `(,y / ,x) a)))
...
(property (x y :saexpr a :assignment)
  :h (and (!= 0 (saeval y a)) (!= *er* (saeval y a)))
  :b (== (saeval `(,x ^ ((2 * ,y) / ,y)) a) (saeval `(,x ^ 2) a)))
...
(defdata
  (aaexpr (or rational
              ...
              aauexpr
              aabexpr))
  (aauexpr (list uoper aaexpr))
  (aabexpr (list aaboper aaexpr aaexpr)))
...
(definec sael->aa (s :saexpr) :aaexpr
  (match s
    ((:or :rational :var) s)
    ((uop s) `(,uop ,(sael->aa s)))
    ((x '^ y) `(expt ,(sael->aa x) ,(sael->aa y)))
    ((x bop y) `(,bop ,(sael->aa x) ,(sael->aa y)))))

(definec aa->sael (a :aaexpr) :saexpr
  (match a
    ((:or :rational :var) a)
    ((uop s) `(,uop ,(aa->sael s)))
    (('expt x y) `(,(aa->sael x) ^ ,(aa->sael y)))
    ((bop x y) `(,(aa->sael x) ,bop ,(aa->sael y)))))
...
(definec aaeval (e :aaexpr a :assignment) :rational
  (match e
    ...
    (:aauexpr
     (('- x) (- (aaeval x a)))
     (('/ x) (let ((x (aaeval x a))) (if (= x 0) x (/ x)))))
    (:aabexpr
     (('- x y) (- (aaeval x a) (aaeval y a)))
     (('+ x y) (+ (aaeval x a) (aaeval y a)))
     (('* x y) (* (aaeval x a) (aaeval y a)))
     (('/ x y) (let ((y (aaeval y a))) (if (= y 0) y (/ (aaeval x a) y))))
     (('expt x y) (let ((x (aaeval x a))
            (y (aaeval y a)))
            (if (or (not (intp y)) (and (< y 0) (= x 0)))
                1
            (expt x y)))))))
...
(property (s :aaexpr a :assignment)
  (let ((av (aaeval s a))
    (sv (saeval (aa->sael s) a)))
    (implies (!= sv *er*) (= sv av))))

;; Extra credit 2 (25 points, all or nothing)
...
;; experience with other theorem provers. Examples of other theorem
