...
(definec lookup (v :var a :assignment) :rational
   (match a
       (nil 1)
     ((f . r) (if (equal v (car f))
      (cdr f)
      (lookup v r)))))

(check= (lookup 'a '((x . 1) (y . 1/2))) 1)
(check= (lookup 'y '((x . 1) (y . 1/2))) 1/2)      
...
(definec eval-uop (op :uoper val :rational) :rat-err
   (match op
       ('- (- val))
     ('/ (if (equal val 0)
       *er*
...
(definec eval-^ (base :rational exp :rational) :rat-err
   (cond ((not (integerp exp)) *er*)
         ((and (equal base 0) (< exp 0)) *er*)
         (t (expt base (ifix exp)))))

(definec eval-bop (op :boper x y :rational) :rat-err
   (match op
       ('+ (+ x y))
     ('- (- x y))
     ('* (* x y))
...
(definec saeval (e :saexpr a :assignment) :rat-err
   ...
   (cond ((rationalp e) e)
         ((varp e) (lookup e a))
         ((usaexprp e) (let ((arg-val (saeval (second e) a)))
             (if (rationalp arg-val)
           (eval-uop (first e) arg-val)
           *er*)))
         (t (let ((x-arg (saeval (first e) a))
      (y-arg (saeval (third e) a)))
        (if (and (rationalp x-arg) (rationalp y-arg))
      (eval-bop (second e) x-arg y-arg)
      *er*)))))
...
(property (x y z :saexpr a :assignment)
    :hyps (not (or (erp (saeval x a)) (erp (saeval y a)) (erp (saeval z a))))
    (== (* (saeval x a) (+ (saeval y a) (saeval z a)))
     (+ (* (saeval x a) (saeval y a)) (* (saeval x a) (saeval z a)))))
...
(property (x y :saexpr a :assignment)
    :hyps (and (not (or (erp (saeval x a)) (erp (saeval y a))))
         (!= (saeval x a) 0)
         (!= (saeval y a) 0))
    (== (/ 1 (/ (saeval x a) (saeval y a)))
     (/ (saeval y a) (saeval x a))))

;; 5. (0 ^ x) = 0, for saexpr x

(property (x :saexpr a :assignment)
    :hyps (and (not (erp (saeval x a)))
         (posp (saeval x a)))
    (== (expt 0 (saeval x a))
     0))
...
(property (x y :saexpr a :assignment)
    :hyps (and (not (or (erp (saeval x a)) (erp (saeval y a))))
         (!= (saeval y a) 0))
    (== (expt (saeval x a) (/ (* 2 (saeval y a)) (saeval y a)))
     (expt (saeval x a) 2)))
...
(defdata
  ...
  (baaexpr (list aboper aaexpr aaexpr)))

;; Define functions sael->aa and aa->sael that transform SAEL
;; and ACL2s arithmetic expressions to each other.

(definec sael->aa (s :saexpr) :aaexpr
   (match s
       (:usaexpr (list (first s) (sael->aa (second s))))
       (:bsaexpr (let ((x (sael->aa (first s)))
           (op (second s))
           (y (sael->aa (third s))))
       (if (equal op '^)
           (list 'expt x y)
...
(definec aa->sael (a :aaexpr) :saexpr 
   (match a
     (:uaaexpr (list (first a) (aa->sael (second a))))
     (:baaexpr (let ((op (first a))
         (x (aa->sael (second a)))
         (y (aa->sael (third a))))
           (if (equal op 'expt)
         (list x '^ y)
...
(definec eval-auop (op :uoper val :rational) :rational
   (match op
     ('- (- val))
     ('/ (if (equal val 0)
       0
...
(definec eval-expt (base :rational exp :rational) :rational
   (cond ((or (equal exp 0) (not (integerp exp))) 1)
         ((equal base 0) 0)
         (t (expt base exp))))

(definec eval-abop (op :aboper x y :rational) :rational
   (match op
       ('+ (+ x y))
     ('- (- x y))
     ('* (* x y))
...
(definec aaeval (e :aaexpr a :assignment) :rational 
   (cond ((rationalp e) e)
         ((varp e) (lookup e a))
         ((uaaexprp e) (let ((arg-val (aaeval (second e) a)))
             (eval-auop (first e) arg-val)))
         (t (let ((x-arg (aaeval (second e) a))
      (y-arg (aaeval (third e) a)))
        (eval-abop (first e) x-arg y-arg)))))
...
(property (s :saexpr a :assignment)
    :hyps (not (erp (saeval s a)))
    (equal (aaeval (sael->aa s) a)
     (saeval s a)))

(property (a :aaexpr ass :assignment)
    :hyps (not (erp (saeval (aa->sael a) ass)))
    (equal (saeval (aa->sael a) ass)
     (aaeval a ass)))