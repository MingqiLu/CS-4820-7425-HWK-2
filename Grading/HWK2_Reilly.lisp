    ...
    (:usaexpr
     ...
     ((equal op '/) (if (= v 0)
                *er*
                (/ 1 v))))))
    (:bsaexpr
     (let ((v1 (saeval (car e) a))
       (op (cadr e))
       (v2 (saeval (caddr e) a)))
       (cond
     ((or (equal v1 *er*) (equal v2 *er*)) *er*)
     ((equal op '+) (+ v1 v2))
     ((equal op '-) (- v1 v2))
     ((equal op '*) (* v1 v2))
     ((equal op '/) (if (= v2 0)
                *er*
                (/ v1 v2)))
     ((equal op '^) (if (or (not (intp v2)) (and (= 0 v1) (< v2 0)))
                *er*
                (expt v1 v2))))))))
...
(property (x y :saexpr a :assignment)
    :hyps (let ((vx (saeval x a)) (vy (saeval y a)))
        (and (not (equal vx *er*))
             (not (equal vy *er*))
             (not (= vx 0))
             (not (= vy 0))))
...
(definec sael->aa (e :saexpr) :aaexpr
     (match e
       (:rational e)
       (:var e)
       (:usaexpr (list (car e) (sael->aa (cadr e))))
       (:bsaexpr
        (if (equal '^ (cadr e))
        (list 'expt (sael->aa (car e)) (sael->aa (caddr e)))
        (list (cadr e) (sael->aa (car e)) (sael->aa (caddr e)))))))

(definec aa->sael (e :aaexpr) :saexpr
     (match e
       (:rational e)
       (:var e)
       (:uaaexpr (list (car e) (aa->sael (cadr e))))
       (:baaexpr
        (if (equal 'expt (car e))
        (list (aa->sael (cadr e)) '^ (aa->sael (caddr e)))
        (list (aa->sael (cadr e)) (car e) (aa->sael (caddr e)))))))
