...
(defdata
  (aaexpr (or rational  ; or is the same as oneof
              var
              uaaexpr   ; unary ACL2s expression
              baaexpr)) ; binary ACL2s expression
  (uaaexpr (list auoper aaexpr))
  (baaexpr (list aboper aaexpr aaexpr)))


(check= (aaexprp '(- (+ x y) (/ z))) t)
(check= (aaexprp '(+ x y z)) nil)
...
(property (e :saexpr a :assignment)
  (implies (not (erp (saeval e a)))
           (== (aaeval (sael->aa e) a)
               (saeval e a))))

(property (e :aaexpr a :assignment)
  (implies (not (erp (saeval (aa->sael e) a)))
           (== (aaeval e a)
               (saeval (aa->sael e) a))))
